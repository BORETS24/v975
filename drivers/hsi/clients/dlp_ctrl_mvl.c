...
static DEFINE_MUTEX(command_tx_mutex);
				...
				unsigned char param2, unsigned char param3)
{
	...
	//Only one command can be process on a single channel at the same time
	mutex_lock(&command_tx_mutex);
...
* @return 0 when OK, error value otherwise
*/
int dlp_ctrl_send_echo_noack(struct dlp_channel *ch_ctx)
{
	int ret;
	unsigned char param1, param2, param3;
