...
static unsigned int vnn_set_flag;
static u8 vnn_save;
#define VNN_DELAY (HZ*3)
void atomisp_vnn_work(struct work_struct *work)
{
	u8 vnn_svid = 0, vnn_spi = 0;
	int ret;
	struct atomisp_device *isp = container_of(work, struct atomisp_device,
						  vnn_work.work);
	if (vnn_set_flag) {
		ret = intel_msic_reg_read(0x92, &vnn_svid);
		if (ret)
			pr_err("msic reg 0x92 read fail\n");
		else if (vnn_svid != vnn_save) {
			if ((vnn_svid & 0x7F) < 0x28)
                        	vnn_spi = 0x24;
                	else
                        	vnn_spi = (vnn_svid - 0x04)|0x80;
			ret = intel_msic_reg_write(0xC1, vnn_spi);
			if (ret)  
				pr_err("msic reg 0xc1 write fail\n");
			else
				vnn_save = vnn_svid;
			pr_info("vnn_svid=0x%x, vnn_spi=0x%x..\n", vnn_svid, vnn_spi);
		}
		schedule_delayed_work(&isp->vnn_work, VNN_DELAY);
	}
}
...
static int atomisp_open(struct file *file)
{
	...
	if (vnn_set_flag == 0) {
		vnn_set_flag = 1;
		vnn_save = 0;
		schedule_delayed_work(&isp->vnn_work, 0);
	}
done:
	pipe->users++;
...
*/
	...
	if (vnn_set_flag) {
		vnn_set_flag = 0;
		cancel_delayed_work_sync(&isp->vnn_work);
		ret = intel_msic_reg_write(0xC1, 0x24);
		if (ret)
			pr_err("msic Reg 0xC1 write fail\n");
	}
done:
